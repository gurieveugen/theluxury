...
class ShopSearchFilterWidget extends WP_Widget {
	...
	function widget($args, $instance){
		...
		<form class="search-filter-form" action="<?php echo get_permalink($instance['search_page']); ?>">
			...
			$shop_cats = $parents + get_categories(array('child_of' => $queried_object->parent, 'exclude' => $instance['exclude-category'], 'order' => 'DESC'));
			...
			$display_tree   = $this->dispalyAllNodes($child_nodes, $father->parent, 0, '<div class="sub-category" style="%s">', "</div>", '<div class="f-row open %s" %s>', "</div>", $display_cats);
	 ...
	 * @return string               - HTML
	 */
	function dispalyAllNodes($child_nodes, $parent_id, $depth = 0, $before_nodes = '<div class="sub-category" style="%s">', $after_nodes = "</div>", $before_node = '<div class="f-row open %s" %s>', $after_node = "</div>", $display_cats = array())
	{		
		global $query_vars;
		var_dump($query_vars);
		$queried_object = get_queried_object();		
		...
		if (isset($child_nodes[$parent_id])) 
		{
		    foreach ($child_nodes[$parent_id] as $id) 
		    {
				...
				$input          = ($depth > 0) ? '<input data-block="shop-by-category" autocomplete="off" type="checkbox" name="filter-category[]" value="'.$cat->slug.'" id="category-'.$cat->term_id.'" rel="'.$rel.'" '.$checked.' />' : '';
					...
					{		
						if($cust_tax->term_id != NULL)
						{
							...
							if ($custom_tax == 'price')
							{
								...
								$block.= '<input data-block="shop-by-'.$custom_tax.'"  autocomplete="off" type="checkbox" name="filter-'.$custom_tax.'[]" value="'.$cust_tax->slug.'" id="'.$custom_tax.'-'.$cust_tax->term_id.'"'.$checked.' />';
