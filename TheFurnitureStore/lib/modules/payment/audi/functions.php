...
function audi_response(){
	...
	switch($feedback['status'])
	{
		case "0":
			...
			// Email to customer 
			$EMAIL->email_confirmation($order,$feedback);

			// Email to shop owner
			$search		= array("[##header##]","[##f_name##]","[##l_name##]","[##amount##]","[##currency##]","[##url##]");
			$replace 	= array($EMAIL->email_header(),$order['f_name'],$order['l_name'],$order['amount'],get_option('wps_currency_code'),url_be());
